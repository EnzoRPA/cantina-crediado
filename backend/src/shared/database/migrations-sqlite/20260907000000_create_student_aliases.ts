...
export async function up(knex: Knex): Promise<void> {
  ...
  if (!exists) {
    await knex.schema.createTable('student_aliases', (table) => {
      ...
      table.text('school_id').notNullable().references('id').inTable('schools').onDelete('CASCADE');
      table.text('student_id').notNullable().references('id').inTable('students').onDelete('CASCADE');
